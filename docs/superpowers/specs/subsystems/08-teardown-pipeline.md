...
1. **`taste/tracks.csv` (ReccoBeats)** — authoritative for track-level facts. When a row exists for the track, its values for `bpm`, `key_camelot`, `key_standard`, `mode`, `energy`, `danceability`, `valence`, `acousticness`, `instrumentalness`, `liveness`, `loudness`, `speechiness`, `genre` are the source of truth. Librosa estimates that disagree are explicitly named as cross-checks (*"librosa reads X, csv reads Y; csv is authoritative"*).
